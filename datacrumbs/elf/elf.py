...
class CorpusReader(et.elffile.ELFFile):  # noqa
    ...
    def __init__(self, filename):
        try:
            ...
            sys.stderr.write(f"Warning: {filename} does not exist.\n")
            self.fd = None
            self.elffile = None
            self.filename = None
            return
    ...
    def __str__(self):
        if self.filename is not None:
            return f"[CorpusReader:{self.filename}]"
    ...
    @property
    def header(self):
        return dict(self.elffile.header) if self.elffile else {}
    ...
    def get_architecture(self):
        return self.elffile.header.get("e_machine") if self.elffile else ""
    ...
    def get_symbols(self):
        """
        Return a set of symbols from symbol tables
        """
        symbols = {}
        ...
        for idx, section in tables:
            ...
            for sym_idx, symbol in enumerate(section.iter_symbols()):
                ...
                symbols[symbol.name] = {
                    ...
                    "value": int(hex(symbol["st_value"], 16)),
    ...
    def _symbol_version(self, idx):
        """
        Get symbol version information.
