...
ConfigurationManager::ConfigurationManager(int argc, char** argv)
  ...
  DC_LOG_DEBUG("[ConfigurationManager] Loading configuration file: %s", config_path.string().c_str());
  try {
    config = YAML::LoadFile(config_path.string());
    DC_LOG_DEBUG("[ConfigurationManager] Configuration file loaded successfully.");
  } catch (const YAML::ParserException& e) {
    DC_LOG_ERROR("[ConfigurationManager] Failed to parse configuration file: %s", config_path.string().c_str());
    throw std::runtime_error("Failed to parse configuration file: " + config_path.string());
  } catch (const YAML::BadFile& e) {
    DC_LOG_ERROR("[ConfigurationManager] Failed to load configuration file: %s", config_path.string().c_str());
  ...
  if (config != nullptr) {
    ...
    if (config[DC_YAML_MODE]) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Mode set from config: %s", config[DC_YAML_MODE].as<std::string>().c_str());
    }
    ...
    if (config[DC_YAML_TRACE_LOG_DIR]) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Trace log dir set from config: %s", this->trace_log_dir.string().c_str());
    }
    ...
    if (config[DC_YAML_DATA_DIR]) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Data directory set from config: %s", this->data_dir.string().c_str());
    } else {
      this->data_dir = DATACRUMBS_DATA_DIR;
      DC_LOG_DEBUG("[ConfigurationManager] Data directory not specified, using default: %s", this->data_dir.string().c_str());
    }
    ...
    if (config[DC_YAML_PROFILING_INTERVAL]) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Profiling interval set from config: %f", this->profiling_interval);
    }
        ...
        if (probe_node["type"]) {
          ...
          case CaptureType::HEADER: {
            ...
            if (probe_node["file"]) {
              ...
              DC_LOG_DEBUG("[ConfigurationManager] Added HEADER probe: %s", header_probe->file.c_str());
            } else {
          ...
          case CaptureType::BINARY: {
            ...
            if (probe_node["file"]) {
              ...
              DC_LOG_DEBUG("[ConfigurationManager] Added BINARY probe: %s", binary_probe->file.c_str());
            } else {
          ...
          case CaptureType::KSYM: {
            ...
            if (probe_node["regex"]) {
              ...
              DC_LOG_DEBUG("[ConfigurationManager] KSYM probe regex set: %s", probe->regex.c_str());
            } else {
          ...
          case CaptureType::USDT: {
            ...
            if (probe_node["binary_path"]) {
              ...
              DC_LOG_DEBUG("[ConfigurationManager] Added USDT probe: %s", usdt_probe->binary_path.c_str());
            } else {
            ...
            if (probe_node["provider"]) {
              ...
              DC_LOG_DEBUG("[ConfigurationManager] USDT provider set: %s", usdt_probe->provider.c_str());
            } else {
          ...
          default:
            DC_LOG_ERROR("[ConfigurationManager] Unknown CaptureType: %s", probe_node["type"].as<std::string>().c_str());
            throw std::invalid_argument("Unknown CaptureType in configuration: " + probe_node["type"].as<std::string>());
          }
          // Parse probe type
          if (probe_node["probe"]) {
            ...
            DC_LOG_ERROR("[ConfigurationManager] Probe type missing for capture type: %s", probe_node["type"].as<std::string>().c_str());
            throw std::invalid_argument("Probe type is required for capture type: " + probe_node["type"].as<std::string>());
          }
          // Parse probe name
          if (probe_node["name"]) {
            probe->name = probe_node["name"].as<std::string>();
          } else {
            DC_LOG_ERROR("[ConfigurationManager] Probe name missing for capture type: %s", probe_node["type"].as<std::string>().c_str());
            throw std::invalid_argument("Probe name is required for capture type: " + probe_node["type"].as<std::string>());
          }
          ...
          if (probe_node["regex"]) {
            ...
            DC_LOG_TRACE("[ConfigurationManager] No regex provided for probe: %s", probe->name.c_str());
          }
    ...
    if (config[DC_YAML_USER]) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] No user specified in config, using default: %s", this->user.c_str());
    }

    // Override config path if provided as argument
    if (parser.config_path) {
      this->path = *parser.config_path;
      DC_LOG_DEBUG("[ConfigurationManager] Config path overridden by argument: %s", this->path.string().c_str());
    }
    ...
    if (parser.trace_log_dir) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Trace log dir overridden by argument: %s", parser.trace_log_dir->c_str());
    }
    ...
    if (parser.profiling_interval) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] Profiling interval overridden by argument: %f", *parser.profiling_interval);
    }
    ...
    if (parser.user) {
      ...
      DC_LOG_DEBUG("[ConfigurationManager] No user specified, using default: %s", this->user.c_str());
    }
  }
...
void ConfigurationManager::derive_configurations() {
  ...
  DC_LOG_DEBUG("[ConfigurationManager] Trace file path: %s", this->trace_file_path.string().c_str());
  ...
  DC_LOG_DEBUG("[ConfigurationManager] Probe file path: %s", this->probe_file_path.string().c_str());
  ...
  DC_LOG_DEBUG("[ConfigurationManager] Probe exclusion file path: %s", this->probe_exclusion_file_path.string().c_str());
  ...
  DC_LOG_DEBUG("[ConfigurationManager] Category map path: %s", this->category_map_path.string().c_str());
}
...
void ConfigurationManager::validate_configurations() {
  ...
  if (!std::filesystem::exists(this->data_dir)) {
    DC_LOG_ERROR("[ConfigurationManager] Data directory does not exist: %s", this->data_dir.string().c_str());
  ...
  if (!std::filesystem::exists(std::filesystem::path(this->trace_log_dir))) {
    DC_LOG_ERROR("[ConfigurationManager] Trace log directory does not exist: %s", this->trace_log_dir.string().c_str());
