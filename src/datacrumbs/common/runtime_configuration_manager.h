...
class RuntimeConfigurationManager {
 public:
  ...
  std::filesystem::path invalid_probe_file_path;
  ...
  std::unordered_map<std::string, std::unordered_set<std::string>> invalid_runtime_probes;

  bool is_known_invalid_runtime_probe(const std::string& probe_name,
                                      const std::string& function_name) const;
  void record_invalid_runtime_probe(const std::string& probe_name,
                                    const std::string& function_name);
  void persist_invalid_runtime_probes() const;

 private:
  void derive_configurations();
  void load_runtime_system_configuration();
  void load_runtime_probe_file();
  void load_invalid_runtime_probes();
